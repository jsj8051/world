// 职责：Territory cohesion (Order 45)（2026-08-19 拆分自 CivModels.cs 纯重构；2026-08-23 概念=机制组合迁移至 Mechanics/<域>/ 目录）
using Godot;
using System;
...
public sealed class TerritoryModel : CivModelBase
{
    ...
    protected override bool CanApply(CivSimContext ctx) =>
        ctx.Tick - ctx.TerritoryLastRebuild >= CivSimContext.TerritoryRebuildEvery;   // 频率守卫（凝聚重算间隔）

    protected override void Apply(CivSimContext ctx)
    {
        ctx.TerritoryLastRebuild = ctx.Tick;   // 频率时间戳（副作用）
        Rebuild(ctx);
    }
