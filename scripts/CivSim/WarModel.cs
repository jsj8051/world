// Responsibility: War (Order 51) - Stage 5 military conquest (docs/阶段5设计-军事征服.md).
using Godot;
using System;
...
public sealed class WarModel : CivModelBase
{
    ...
    private static float PowerOf(CivSimContext ctx, int stateId, War w, Tribe[] byId, Dictionary<int, Settlement> settleById)
    {
    ...
    private static void ApplyLoss(CivSimContext ctx, int loserState, Tribe[] byId)
    {
    ...
    private static void Annex(CivSimContext ctx, War w, int winner, int loser, Tribe[] byId)
    {
        ...
        if (chief != null && !chief.Dead && chief.Cell >= 0 && chief.Cell < ctx.Grid.N)
        {
            ...
            if (target >= 0)
            {
                if (ctx.CellTribes[chief.Cell] == chief) ctx.CellTribes[chief.Cell] = null;
                chief.Cell = target;
                chief.LastMigrateTick = ctx.Tick;
                ctx.CellTribes[target] = chief;
                ctx.Migrations++;
            }
        }
    ...
    private static void CedeCells(CivSimContext ctx, War w, int loser, Tribe[] byId)
    {
        ...
        for (int c = 0; c < n && ceded < CivSimContext.WarCedeCells; c++)
        {
            ...
            var oTribe = o < byId.Length ? byId[o] : null;
            bool isLoser = o == loser || (oTribe != null && !oTribe.Dead && oTribe.ChiefdomId == loser);
            if (!isLoser) continue;
            // 边境判定：任一邻居属战胜国
    ...
    internal static bool CanDeclare(CivSimContext ctx, Tribe a, Tribe b, float reachKm)
    {
    ...
    private static bool StatesTouch(CivSimContext ctx, Tribe a, Tribe b)
    {
        var byId = IdIndex(ctx);
    ...
    private static Tribe[] IdIndex(CivSimContext ctx)
    {
