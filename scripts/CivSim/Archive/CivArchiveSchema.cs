...
///
/// 设计要点：
/// 1. **单源真值**：Polity 持久字段在此集中声明，Write/Read 由表驱动 → 不可能一处写一处漏。
...
///    不再有版本兼容分支——旧档全删。）
/// 3. **反射校验**：Validate() 检查每项 Name 在 Polity 上真实存在（字段改名后清单过期 → 测试红）。
...
public static class CivArchiveSchema
{
    ...
    ///   与 Polity partial 分区对齐）——旧 v15 档作废（用户拍板），读端拒绝 Older。</summary>
    public readonly record struct FieldDef(
        string Name,
        int SinceVer,
        Action<ChunkWriter, Polity> Write,
        Func<ChunkReader, Polity, bool> Read);

    // ══════════════════════════════════════════════════════════════════
    // Polity 字段清单（v16 概念分组，2026-08-23 拍板重排）：
    //   Core 组（Polity.cs：身份/人口/科技/冷却/商品/聚落关联/份额场）
    //   Chiefdom 组（Polity.Chiefdom.cs：声望/贡赋/继承窗口/政治归属）
    //   State 组（Polity.State.cs：国家/军事冷却）
    // 字节序 = 组序 + 组内顺序（与 .cmp/.mpa CIVI 段字节布局一致；读端仅接受 v16+）
    // ══════════════════════════════════════════════════════════════════
    public static readonly FieldDef[] PolityFields =
    {
        // ── Core 组：身份与人口 ──
    ...
    /// <summary>反射校验：清单字段名必须在 Polity 上真实存在（防字段改名/删除后清单过期静默失效）。
    /// 返回 false = 清单过期（写档调用方拒绝写入防静默漏字段）；LogErr 报具体字段名（定位清单过期点）。</summary>
    public static bool Validate()
    {
        foreach (var def in PolityFields)
        {
            if (def.Write == null || def.Read == null) continue;   // 特例字段（TechKeys 内联读写）
            if (typeof(Polity).GetField(def.Name, BindingFlags.Public | BindingFlags.Instance) == null)
            {
                LogService.LogErr("CivArchiveSchema", $"字段 '{def.Name}' 在 Polity 上不存在——清单过期！");
                return false;
            }
        }
        return true;
