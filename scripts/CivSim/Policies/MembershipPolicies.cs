...
{
    public int Assign(CivSimContext ctx, Polity e, Polity[] byId, int[] bestChief) => e.ConqueredBy;
}

/// <summary>酋长归属策略：自己 = 自己酋邦的中心（ChiefdomId = 自身 Id）。</summary>
public sealed class ChiefMemberPolicy : IMembershipPolicy
{
    public int Assign(CivSimContext ctx, Polity e, Polity[] byId, int[] bestChief) => e.Id;
}

/// <summary>自由成员归属策略：选 ChiefReach 内 Prestige 最高的酋长（庇护人；-1 = 独立）。</summary>
public sealed class FreeMemberPolicy : IMembershipPolicy
{
    public int Assign(CivSimContext ctx, Polity e, Polity[] byId, int[] bestChief)
...
public static class MembershipPolicies
{
    ...
    public static IMembershipPolicy Of(CivSimContext ctx, Polity e, Polity[] byId)
    {
        if (e.ConqueredBy >= 0)
        {
