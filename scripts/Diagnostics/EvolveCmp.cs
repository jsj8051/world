...
public partial class EvolveCmp : DiagSceneBase
{
    public override void _Ready()
    {
        ...
        LogService.Log("EvolveCmp", $"演化完成 {result.FinalTick} tick（{result.FinalTick * CivSimContext.TickYears} 年）| 实体 {ctx.Polities.Count} | 人口 {ctx.TotalPopulation():F0} | 首转农 tick {ctx.FirstFarmTick} | 耗时 {sw.ElapsedMilliseconds}ms" +
