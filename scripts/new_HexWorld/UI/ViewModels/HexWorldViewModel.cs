...
// 边界描边格边集（2026-09-09 起：边粒度记账，弃角点粒度——同板边被相邻边界边角点污染整条误描）、
...
// 边界描边格边集：(格 id, 顶点对) = 异板共享边在边界两侧格的名下记账。View 据此把每格该边的
// 边界旗（UV2.y）置 1，描边带宽由片元侧边距权重决定。数据源 = H3Plate.ExtractBoundaryCellEdges
