...
public partial class MapViewer : Node3D
{
    ...
    private static int PowerIdOf(Polity e)
    {
        ...
        // ⚠️ 2026-08-18 修复：band 也进独立域（0x20000000）——实体 Id 从 0 分配（NextPolityId 起始 0），
    ...
    private static byte PolityOf(Polity e)
    {
    ...
    private void PrecomputeTileValues(MapData map, List<HexTile> tiles, System.Threading.CancellationToken token, Action<float> progress = null)
    {
        ...
        var civIdMap = new System.Collections.Generic.Dictionary<int, Polity>();
        if (hasCiv)
            foreach (var ce in _civCtx.Polities)
        ...
        System.Threading.Tasks.Parallel.For(0, n, i =>
        {
            ...
            if (hasCiv)
            {
                ...
                if (ownerId >= 0 && civIdMap.TryGetValue(ownerId, out var dom))
                {
                    ...
                    //   人口只在驻扎格（Polity.Cell）显示该 band 的 P（并行循环后实体表直写）。
                    ...
                    _cache.TilePolityKind[i] = PolityOf(dom);
        ...
        if (hasCiv)
        {
            ...
            for (int e = 0; e < _civCtx.Polities.Count; e++)
            {
                var ce = _civCtx.Polities[e];
    ...
    private void ClickDebug(int i)
    {
        ...
        GD.Print($"  elev={_cache.TileElev[i]:F3} 海拔={elevM2:F0}m pop={_cache.TilePop[vid]:F1} power={_cache.TilePower[i]} polity={_cache.TilePolity[i]} kind={_cache.TilePolityKind[i]} terr={_cache.TileTerritory[i]} culture={_cache.TileCulture[i]} religion={_cache.TileReligion[i]}");
